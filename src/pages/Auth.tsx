...
export default function Auth() {
  ...
  const handleForgotPassword = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!email) return toast.error("Bitte gib deine E-Mail-Adresse ein");
    setLoading(true);
    try {
      ...
      toast.success("Passwort-Reset-Link wurde gesendet! Prüfe dein E-Mail-Postfach.", { duration: 8000 });
        ...
        <div className="w-full max-w-md animate-fade-in">
          ...
          <div className="bg-card rounded-2xl p-8 shadow-lg border border-border">
            {isForgotPassword ? (
              <>
                ...
                <form onSubmit={handleForgotPassword} className="space-y-4">
                  <div className="space-y-2">
                    ...
                    <Input id="reset-email" type="email" required value={email} onChange={e => setEmail(e.target.value)} placeholder="name@beispiel.de" />
                  </div>
              ...
              <>
                {/* Google Sign-In */}
                <Button variant="outline" className="w-full mb-4 gap-2" onClick={handleGoogleSignIn} disabled={googleLoading}>
                  {googleLoading ? (
                ...
                <form onSubmit={handleSubmit} className="space-y-4">
                  ...
                  {!isLogin && (
                    <label className="flex items-start gap-2 cursor-pointer">
                      <input type="checkbox" checked={agbAccepted} onChange={(e) => setAgbAccepted(e.target.checked)} className="mt-1 w-4 h-4 rounded accent-primary" />
